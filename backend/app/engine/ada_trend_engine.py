"""
ADA Trend Following Engine
+1195% backtested return (+155.7%/year)
Same strategy as SOL/MATIC/DOGE - Trend 2d cooldown
"""
...
class ADATrendEngine:
    ...
    Backtested: +1195% over 7.68 years (~155.7%/year)
    """
    ...
    def __init__(self):
        ...
        # State tracking
        self.current_trend = ADATrend.RANGE
    ...
    def _detect_raw_trend(self, sma10: float, sma20: float, sma50: float) -> ADATrend:
        """Detect trend from SMA alignment"""
        if sma10 > sma20 > sma50:
            return ADATrend.UPTREND
        elif sma10 < sma20 < sma50:
            return ADATrend.DOWNTREND
        return ADATrend.RANGE
    
    def _update_trend(self, raw_trend: ADATrend) -> None:
        ...
        if raw_trend == self.current_trend:
            ...
            if self.trend_counter >= self.trend_confirm_hours:
                logger.info(f"ADA Trend changed: {self.current_trend.value} -> {raw_trend.value}")
    ...
    def get_signal(
        ...
        if self.in_position:
            ...
            if self.current_trend == ADATrend.UPTREND and can_trade:
                # RSI Top Exit
            ...
            elif self.current_trend == ADATrend.DOWNTREND:
                self.last_trade_time = datetime.now()
        ...
        # ========== NO POSITION ==========
            ...
            elif self.in_cash_bear and self.current_trend == ADATrend.UPTREND and can_trade:
                self.last_trade_time = datetime.now()
                self.in_cash_bear = False
                self.highest_since_entry = current_price
                return ADASignal(
